...
$(document).ready(function() {
  ...
  if (collection_type == "hoard") {
    ...
    var tileLayers = {
      ...
      imperium: L.tileLayer(
        "http://dare.ht.lu.se/tiles/imperium/{z}/{x}/{y}.png",
        {
          maxZoom: 11,
          attribution:
            'Powered by <a href="http://leafletjs.com/">Leaflet</a>. Map base: <a href="http://dare.ht.lu.se/" title="Digital Atlas of the Roman Empire, Department of Archaeology and Ancient History, Lund University, Sweden">DARE</a>, 2015 (cc-by-sa).'
        }
      ),

      mb_physical: L.tileLayer(
    ...
    var map = new L.Map("mapcontainer", {
      ...
      zoom: 4,
      layers: [tileLayers[baselayers[0]]] || tileLayers.osm
    });

    //add mintLayer from AJAX
    var mintLayer = L.geoJson
      .ajax(path + "mints.geojson?q=" + q, {
        pointToLayer: renderPoints
      })
      .addTo(map);

    var subjectLayer = L.geoJson
      .ajax(path + "subjects.geojson?q=" + q, {
        pointToLayer: renderPoints
      })
      .addTo(map);
    ...
    for (i = 0; i < baselayers.length; i++) {
      ...
      switch (baselayers[i]) {
        case "osm":
          ...
          break;
        case "imperium":
          label = "Imperium Romanum";
          break;
        case "mb_physical":
          label = "Terrain and Streets";
          break;
      }
  ...
  $(
    ...
    "click",
    ".paging_div .page-nos .btn-toolbar .pagination a.pagingBtn",
    function(event) {
  ...
  function refreshMap() {
    ...
    if (collection_type == "hoard") {
      $("#timemap").html(
        '<div id="mapcontainer" class="fullscreen"><div id="map"/></div><div id="timelinecontainer"><div id="timeline"/></div>'
      );
  ...
  function renderPopup(e) {
    ...
    query +=
      " AND " +
      e.layer.feature.properties.type +
      '_uri:"' +
      e.layer.feature.properties.uri +
      '"';
    var str =
      e.layer.feature.properties.type +
      ": <a href='#results' class='show_coins' q='" +
      query +
      "'>" +
      e.layer.feature.properties.name +
      "</a> <a href='" +
      e.layer.feature.properties.uri +
      "' target='_blank'><span class='glyphicon glyphicon-new-window'/></a>";
